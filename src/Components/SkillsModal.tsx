...
const SkillsPage: React.FC<SkillsPageProps> = ({ isSkillsOpen, onToggleSkills, isProjectPanelOpen }) => {
  ...
  return (
    <>
      {/* Fixed Skills Button */}
      <div className={`radial-menu-button-container skills ${isProjectPanelOpen ? 'project-open' : ''}`}>
        <button 
      ...
      {/* Skills Page Content */}
      <div className={`skills-page ${isSkillsOpen ? 'open' : ''}`} style={{
        '--brand-colour': currentTheme.colors.primary
      } as React.CSSProperties}>
        {/* Close Button */}
        <button className="skills-close-btn" onClick={onToggleSkills}>
          {/* @ts-ignore */}
          <FiX />
        </button>
                ...
                <div className="core-areas">
                  ...
                  <div className="areas-grid">
                    {skillCategories.map((category) => (
                      <div key={category.id} className="area-card" style={{ '--category-color': category.color } as React.CSSProperties}>
                        <h3>{category.name}</h3>
                        <div className="area-skills">
                          {category.keySkills.slice(0, 3).map(skill => (
                            <span key={skill} className="area-skill">{skill}</span>
                          ))}
                        </div>
                      </div>
                    ))}
